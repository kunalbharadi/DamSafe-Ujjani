...
type NumericalRun = {
  id: string; state: string; input: { request: { engine: string; case_kind: string }; evidence_status?: string; case_classification?: string };
...
function ResultsView({ run, metadata, raw, area, location, maxDepth, onLoad, onExport }: { run?: NumericalRun; metadata: ResultMetadata | null; raw: Window | null; area: ProductArea | null; location: LocationSeries | null; maxDepth: number | null; onLoad: () => void; onExport: (format: string) => void }) {
  ...
  const isSiteApprox = metadata.case_kind === 'SITE_SCENARIO' || run.input.case_classification === 'UJJANI_APPROXIMATE_DEMONSTRATION';
  return <><section className="alert"><strong>{metadata.input_mode} · {metadata.case_kind}</strong> — {metadata.engine} output, source run {metadata.source_run_id}. {metadata.cached ? 'This entry is a verified identical-configuration cache.' : 'This is a retained saved numerical result.'}
      {isSiteApprox && <div style={{ marginTop: '0.5rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}><span className="tag">UJJANI APPROXIMATE DEMONSTRATION</span><span className="tag">NOT VALIDATED</span><span className="tag">APPROXIMATE CHANNEL GEOMETRY</span></div>}
    </section>
